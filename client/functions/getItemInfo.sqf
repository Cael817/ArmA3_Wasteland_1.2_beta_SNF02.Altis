		...
		switch (_itemEntry select 3) do
		{
			...
			case "uni":
			{
				switch (true) do
				{
					...
					{
						//_name = _itemText;
						_description = "Disguise as a Bush Wookie";
					};
			...
			//Cael817, SNAFU, Added description for the "re locker"
			case "object":
			{				
				switch (true) do
					{
					case (["Base Locker", _itemText] call fn_findString != -1):
					{
						//_name = _itemText;
						_description = "Put this down to have the option to lock or ""re lock"" all base objects at once as long as they are bought or locked once before if found, PIN is emty!";
					};
				};
			};
			//Cael817, SNAFU, End
