...
#define IS_FRIENDLY_PLAYER(UNIT) (isPlayer UNIT && {(group UNIT == group player || (!_isIndie && side group UNIT == playerSide)) && {("ItemGPS" in assignedItems UNIT) || ({_x find "_UavTerminal" != -1} count assignedItems UNIT > 0)}}) //Cael817, SNAFU, Make it necessary to have a GPS or UAV Terminal for markers to show.
...
A3W_mapDraw_thread = [] spawn
{
	...
	waitUntil
	{
		...
		if (_showPlayers && {_x in ["ItemGPS","B_UavTerminal","O_UavTerminal","I_UavTerminal"]} count assignedItems player > 0) then
		{
