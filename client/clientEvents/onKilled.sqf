...
//Cael817, Adds personal R.I.P. marker function from GoT
...
if (_player == player && (playerSide == side group _killer) && (player != _killer) && (vehicle player != vehicle _killer)) then
{
	...
	if (playerSide in [BLUFOR,OPFOR]) then
	{
		if (_killer isKindOf "CAManBase") then
		{
			...
			pvar_PlayerTeamKilled = _player; //Cael817, SNAFU, For the bounty function from GoT
			pvar_PlayerTKPrevBounty = _player getVariable ["cbounty", 0]; //Cael817, SNAFU, For the bounty function from GoT
		}
		else
		{
	...
	else // Compensate negative score for indie-indie kills
	{
		if (isPlayer _killer) then
		{
			...
			//Cael817, For the bounty function from GoT
			...
			//Cael817, Lets see if we can pay the reward too
			_kmoney = _killer getVariable ["cmoney", 0];
			_reward = _kmoney + _pbounty;
			_killer setVariable ["cmoney", _reward, true];
			//hint format ["%4 got $%1 reward and $%2 bounty on for killing you", _reward, _bounty, _player, _killer];
		};
	};
	//Cael817, For the bounty function from GoT
	}
	...
	// not in the same team or group? handle bounty!
		//if !(isNull _killer && (player != _killer)) then
