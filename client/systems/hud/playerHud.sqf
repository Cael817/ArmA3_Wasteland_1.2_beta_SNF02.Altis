...
_bountySystem = ["A3W_bountyEnabled"] call isConfigOn;
...
while {true} do
{
	...
	if (_bountySystem) then {//Cael817, SNAFU, Add bounty icon and value
	_strArray pushBack format ["%1 <img size='0.7' image='client\icons\bounty.paa'/>", [player getVariable ["cbounty", 0]] call fn_numbersText];
	};
