...
{
	private ["_veh", "_vehicleID", "_class", "_pos", "_dir", "_vel", "_flying", "_damage", "_fuel", "_hitPoints", "_owner", "_variables", "_textures", "_weapons", "_magazines", "_items", "_backpacks", "_turretMags", "_turretMags2", "_turretMags3", "_ammoCargo", "_fuelCargo", "_repairCargo", "_hoursAlive", "_hoursUnused", "_valid", "_lockState"];
	...
	if (!isNil "_class" && !isNil "_pos" && {count _pos == 3 && (_maxLifetime <= 0 || _hoursAlive < _maxLifetime) && (_maxUnusedTime <= 0 || _hoursUnused < _maxUnusedTime)}) then
	{
		...
		_veh setVariable ["vehSaving_hoursUnused", _hoursUnused, true];//Cael817, Broadcast vehSaving_hoursUnused
		...
		_veh setFuel _fuel;
		_veh lock _lockState; //Cael817, Added line for restoring vehicle lock to its previous state
		
		if (_lockstate >= 2) then
		{
			_veh setVariable ["R3F_LOG_Disabled", true, true]; //Cael817, Added so R3F_LOG 3.x works, needs more code to follow lockstate
			}else{
			_veh setVariable ["R3F_LOG_Disabled", false, true]; //Cael817, Added so R3F_LOG 3.x works, needs more code to follow lockstate
		};
