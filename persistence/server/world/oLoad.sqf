...

{
	...
	if (!isNil "_class" && !isNil "_pos" && {_maxLifetime <= 0 || _hoursAlive < _maxLifetime}) then
	{
		...
		_obj setVariable ["baseSaving_hoursAlive", _hoursAlive, true]; //Cael817, Broadcast baseSaving_hoursAlive
		_obj setVariable ["baseSaving_spawningTime", diag_tickTime];
		_obj setVariable ["objectLocked", true, true]; // force lock
		_obj setVariable ["R3F_LOG_Disabled", false, true]; //Cael817, Added so R3F_LOG 3.x works
...

		{
			...
			switch (_var) do
			{
				...
				case "lockDown": { _value }; //Cael817, Added lockDown
				case "password": { _value }; //Cael817, Added password
				case "ownerName":
				{
