...
_vel = velocity _veh;
_fuel = fuel _veh;
_lockState = locked _veh; //Cael817, Added _lockState
_damage = damage _veh;
_hitPoints = [];
...
_ownerName = _veh getVariable ["ownerName", ""];
...
_lockDown = _veh getVariable "lockDown";

if (!isNil "_lockDown") then {
	_variables pushBack ["lockDown", _lockDown];
};

_password = _veh getVariable ["password", ""];
...
[
	...
	["Velocity", _vel],
	["Fuel", _fuel],
	["LockState", _lockState],  //Cael817, Added LockState
	["Damage", _damage],
