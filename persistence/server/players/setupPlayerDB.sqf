...
"pvar_savePlayerData" addPublicVariableEventHandler
{
	(_this select 1) spawn
	{
		...
		if (!isNull _player && alive _player && !(_player call A3W_fnc_isUnconscious)) then
		{
			...
			_info pushBack ["Bounty", _player getVariable ["cbounty", 0]]; //Cael817, Added Bounty
...
"pvar_requestPlayerData" addPublicVariableEventHandler
{
	(_this select 1) spawn
	{
		...
		[[_this, _data],
		{
			...
			{
				if (_x select 0 == "BankMoney") then //Cael817, changed "exitWith" to "then"
				{
					_player setVariable ["bmoney", _x select 1, true];
				};
				//Cael817, Added bounty and gear level
				if (_x select 0 == "Bounty") then
				{
					_player setVariable ["cbounty", _x select 1, true];
				};
