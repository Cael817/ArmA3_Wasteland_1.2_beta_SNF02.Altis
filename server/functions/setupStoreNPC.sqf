...
if (hasInterface) then
{
	...
	//_npc addAction ["<img image='client\icons\money.paa'/> Sell last vehicle contents", "client\systems\selling\sellLoadedObjects.sqf", [], 0.97, false, true, "", STORE_ACTION_CONDITION + " && " + SELL_VEH_CONTENTS_CONDITION]; //Cael817, Added sell vehicle contents
	_npc addAction ["<img image='client\icons\money.paa'/> Sell last Vehicle", "client\systems\selling\sellVehicle.sqf", [], 0.97, false, true, "", STORE_ACTION_CONDITION + " && " + SELL_VEH_CONTENTS_CONDITION]; //Cael817, Added sell vehicle
	_npc addAction ["<img image='client\icons\money.paa'/> Change owner on last vehicle", "client\systems\selling\changeOwner.sqf", [], 0.97, false, true, "", STORE_ACTION_CONDITION + " && " + SELL_VEH_CONTENTS_CONDITION]; //Cael817, Added change owner
	_npc addAction ["<img image='client\icons\repair.paa'/> Service last Vehicle", "client\systems\selling\serviceVehicle.sqf", [], 0.97, false, true, "", STORE_ACTION_CONDITION + " && " + SELL_VEH_CONTENTS_CONDITION]; //Cael817, Added service vehicle
};
				...
				switch (toLower _type) do
				{
					...
					case "uniform":
					{
						if (_classname != "") then
						{
							...
							_npc forceAddUniform _classname; //Cael817, forceAddUniform if not allowed from loudNL
						};
					};
					case "switchMove":
					{
