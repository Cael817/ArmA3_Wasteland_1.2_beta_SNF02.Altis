...
A3W_fnc_setLockState = { (objectFromNetId (_this select 0)) lock (_this select 1) } call mf_compile; //Cael817, For locking and breaking in
A3W_fnc_reboot = {"Reboot" call BIS_fnc_endMission} call mf_compile; //Cael817, from micovery, lets see if its possible to use it later on
